...
func AddCommentRoutes(router fiber.Router) {
	...
	api.Post("/resource/:resourceID", createResourceComment, middleware.NewRequestLimiter(100, 24*time.Hour), middleware.NewRequestLimiter(10, 1*time.Minute))
	api.Post("/reply/:commentID", createReplyComment, middleware.NewRequestLimiter(200, 24*time.Hour))
